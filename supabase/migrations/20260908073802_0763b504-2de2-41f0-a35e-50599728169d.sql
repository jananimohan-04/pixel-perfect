...
begin
  ...
  select count(*) = 1 into _first from public.profiles;
  select id into _role_id from public.roles where key = case when _first then 'super_admin' else 'viewer' end;
  insert into public.user_roles (user_id, role_id) values (new.id, _role_id) on conflict do nothing;
  return new;
end $$;
...
create index documents_party_idx on public.documents(party_id);
create index documents_search_idx on public.documents(document_number, part_number, document_name);
...
returns boolean language sql stable security definer set search_path = public as $$
  select
    ...
    or not exists (select 1 from public.document_permissions dp where dp.document_id = _document_id)
    or exists (
      select 1 from public.document_permissions dp
      where dp.document_id = _document_id
        and (
          dp.user_id = _user_id
          or dp.role_id in (select role_id from public.user_roles where user_id = _user_id)
          or dp.department = (select department from public.profiles where user_id = _user_id)
        )
    )
$$;
...
create policy "user_roles managed" on public.user_roles for all to authenticated
  using (public.has_permission(auth.uid(),'manage_users')) with check (public.has_permission(auth.uid(),'manage_users'));

create policy "parties readable" on public.parties for select to authenticated using (public.has_permission(auth.uid(),'view'));
create policy "parties managed" on public.parties for all to authenticated
  using (public.has_permission(auth.uid(),'manage_parties')) with check (public.has_permission(auth.uid(),'manage_parties'));

create policy "parts readable" on public.parts for select to authenticated using (public.has_permission(auth.uid(),'view'));
create policy "parts managed" on public.parts for all to authenticated
...
create policy "versions readable" on public.document_versions for select to authenticated
...
create policy "versions insert" on public.document_versions for insert to authenticated
  with check (public.has_permission(auth.uid(),'upload'));
create policy "versions update" on public.document_versions for update to authenticated
...
create policy "doc perms readable" on public.document_permissions for select to authenticated
  using (public.has_permission(auth.uid(),'view'));
create policy "doc perms managed" on public.document_permissions for all to authenticated
...
-- ============ updated_at triggers ============
create trigger t_roles_updated before update on public.roles for each row execute function public.update_updated_at_column();
...
create trigger t_parts_updated before update on public.parts for each row execute function public.update_updated_at_column();
...
-- ============ sample data ============
insert into public.parties (name, code, contact_person, email, phone, address, status) values
...
insert into public.parts (party_id, part_number, part_name, drawing_number, drawing_type, current_revision, status)
select p.id, v.pn, v.nm, v.dn, v.dt, v.rev, v.st::public.doc_status from public.parties p
join (values
...
insert into public.documents (party_id, part_id, document_number, document_name, drawing_number, part_number, document_type, category, description, current_version, status, file_type, created_by_name, updated_by_name, created_at, updated_at)
...
join public.parties pa on pa.id = pt.party_id
join (values
...
insert into public.document_versions (document_id, version_number, file_name, file_type, file_size, revision_notes, status, uploaded_by_name, uploaded_at)
select d.id, g.v,
...
insert into public.audit_logs (user_name, action, document_id, document_number, version_number, party_name, details, ip_address, created_at)
select v.un, v.act, d.id, d.document_number, d.current_version, pa.name, v.det, '10.0.0.' || v.ipn, now() - (v.mins || ' minutes')::interval
from public.documents d
join public.parties pa on pa.id = d.party_id
join (values
...
insert into public.notifications (user_id, title, body, type, document_id, read, created_at)
